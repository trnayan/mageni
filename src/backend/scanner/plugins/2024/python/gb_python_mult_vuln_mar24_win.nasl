...
if(description)
{
  ...
  script_name("Python Multiple Vulnerabilities (Mar 2024) - Windows");
  ...
  - No CVE: urllib.request resolves the hostname before checking it against the system's proxy
  bypass list on Windows

  - No CVE: .pth files with names starting with a dot or containing the hidden file attribute are
  not skipped

  - No CVE: iso2022_jp_3 and iso2022_jp_2004 codecs read out of bounds

  - No CVE: ssl.SSLContext.cert_store_stats() and ssl.SSLContext.get_ca_certs() don't correctly
  lock access to the certificate store, when the ssl.SSLContext is shared across multiple threads");

  script_tag(name:"affected", value:"Python version 3.8.18 and prior, 3.9.x through 3.9.18, 3.10.x
  through 3.10.13, 3.11.x through 3.11.8 and 3.12.x through 3.12.2.");

  script_tag(name:"solution", value:"Update to version 3.8.19, 3.9.19, 3.10.14 or later. For
  version 3.11.x and 3.12.x see the referenced vendor advisories for patches / fixed releases.");
...
if (version_in_range(version: version, test_version: "3.11", test_version2: "3.11.8") ||
    version_in_range(version: version, test_version: "3.12", test_version2: "3.12.2")) {
